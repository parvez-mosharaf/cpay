-- ============================================================
-- Boltpay — 0045: Schedule the reconciliation job
-- ============================================================
-- Runs the reconcile function once a day and alerts if BTCPay and the
-- Boltpay ledger disagree about what settled.
--
...
begin
  ...
  if v_url is null or v_secret is null then
    raise notice 'Vault secrets boltpay_functions_url / boltpay_cron_secret are missing — skipping schedule.';
    return;
  end if;

  -- Unschedule first so re-running this migration cannot stack duplicates.
  perform cron.unschedule('boltpay-reconcile')
  where exists (select 1 from cron.job where jobname = 'boltpay-reconcile');
  ...
  perform cron.schedule(
    ...
    format(
      ...
      $job$,
      -- boltpay_functions_url is the project base URL; the existing
  ...
  raise notice 'Scheduled boltpay-reconcile at 04:10 UTC daily.';
end $$;
...
--
-- Expect four Boltpay jobs:
--   boltpay-reconcile       10 4 * * *
