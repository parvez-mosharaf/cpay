...
Deno.serve(async (req) => {
    ...
    // BTCPay that omits the field would otherwise have every settlement
    // refused, which is a far worse outcome than the narrow risk this closes.
    // It alerts instead, so the gap is visible rather than silent.
    if (eventStoreId && payment.btcpay_store_id && eventStoreId !== payment.btcpay_store_id) {
        console.error(
    ...
    if (!eventStoreId) {
        console.warn(`Webhook for invoice ${invoiceId} carried no storeId — ownership not verified`);
    }
