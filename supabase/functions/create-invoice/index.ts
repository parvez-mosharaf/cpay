...
// Counted off payments.user_id directly rather than through an embedded
// !inner join on payment_links. The join would work, but embedded
// filters combined with head+count have documented rough edges, and
// this is an unauthenticated endpoint that creates real invoices — not
// somewhere to rely on subtle query semantics. payments.user_id is set
// on insert a few lines below and is indexed (idx_payments_user).
const { count: recentCount, error: rateErr } = await supabaseAdmin
.from("payments")
.select("id", { count: "exact", head: true })
.eq("user_id", link.user_id)
.gte("created_at", oneMinuteAgo);
// Fails CLOSED. A broken count query must not become an open door on an
// unauthenticated endpoint that creates real invoices on a real node.
if (rateErr) {
console.error("rate-limit check failed, refusing:", rateErr.message);
