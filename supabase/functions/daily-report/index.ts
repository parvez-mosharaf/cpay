...
async function computeDay(dateStr: string, marginPercent: number) {
  ...
  // Same formula as admin_global_stats() in migration 0017.
  //
  // This used to be `totalSettled * (sell_rate - buy_rate)` from the
  // exchange_rates setting. Those two rates are seeded equal, so this
  // number was always exactly 0 while the header stat — computed from
  // profit_margin_percent — showed something else. Two formulas for one
  // figure meant the Earnings tab and the dashboard card could never agree.
  //
  // The `/ 2` is carried over verbatim from admin_global_stats. If that
  // divisor is wrong, it is wrong in both places now, which is the point:
  // change it once and both follow.
  const adminProfit = (totalSettled * (marginPercent / 100)) / 2;
...
Deno.serve(async (req) => {
  ...
  const { data: marginSetting } = await supabase
    ...
    .maybeSingle();
  const marginPercent = Number(marginSetting?.value?.percent ?? 7.7);
  ...
  for (let i = 0; i < days; i++) {
    ...
    try {
      results.push(await computeDay(dateStr, marginPercent));
    } catch (e) {
  ...
  return new Response(
    JSON.stringify({ marginPercent, days, results, failures }),
    {
